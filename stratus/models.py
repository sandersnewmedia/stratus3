...
from stratus.resizers import resizers
...
class Block(models.Model):
    ...
    def render(self, request=None):
        if not self.content_type:
            ...
            content = render_to_string('stratus/block_%s.html' % self.content_type, {'block': self})

        if request and '_preview' in request.GET and request.user.is_staff:
            return render_to_string('stratus/block_wrapper.html', {
                'block': self,
                'content': content,
            })
        else:
